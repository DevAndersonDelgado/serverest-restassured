...
class BuscarCarrinhoPorIDTest extends BaseAuth {
    ...
    @Test
    void testValidarBuscaDeCarrinhoComSucesso() {
        String carrinhoId = carrinhoClient.cadastrarCarrinho(carrinhosRequest, token)
            .then()
                .statusCode(HttpStatus.SC_CREATED)
                .extract()
                .path("_id")
        ...
        carrinhoClient.buscarCarrinho(carrinhoId, token)
            .then()
                .statusCode(HttpStatus.SC_OK)
            ;
    }
    ...
    @Test
    void testValidarContratoBuscarCarrinhoComSucesso() {
        String carrinhoId = carrinhoClient.cadastrarCarrinho(carrinhosRequest, token)
            .then()
                .statusCode(HttpStatus.SC_CREATED)
                .extract()
        ...
        carrinhoClient.buscarCarrinho(carrinhoId, token)
            .then()
                .statusCode(HttpStatus.SC_OK)
                .body(matchesJsonSchemaInClasspath("schemas/carrinhos/carrinhos_por_id.json"))
            ;
    }
